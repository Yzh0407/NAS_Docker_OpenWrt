...
找到自己所用的网卡，可以根据NAS的ip判断，例如NAS的ip是192.168.3.212，找到带有这个的网卡，复制前面的名称，我的名称就是`enp1s0`

#### 3、docker network create -d macvlan --subnet=192.168.x.0/24 --gateway=192.168.x.1 -o parent=网卡名 自定义macvlan名 (创建macvlan虚拟网卡) （根据自己需求改）

示例：

```
docker network create -d macvlan --subnet=192.168.3.0/24 --gateway=192.168.3.1 -o parent=enp1s0 OpenWrt_Network
```
...
```
docker run --restart always --name OpenWrt -d --network OpenWrt_Network --privileged zzsrv/openwrt:latest /sbin/init
```
...
注意：vi /etc/config/network后除了改ip以外还要再新增option gateway '192.168.3.1'和option dns '192.168.3.1'，不然会出现网络错误
...
提醒一下，容器跑起来之后赶紧进openwt的挂载点，把所有勾全下了，不然会跟fnos的挂载文件系统冲突，导致nas里文件管理的目录消失！！！

进openwrt的界面，左边栏的系统一栏里有个挂载点，点进去，打勾的全取消，避免一个硬盘被两个系统重复挂载，如果文件系统还有冲突就在下面已经挂载的文件系统里把冲突的挑出来删掉，一般openwrt比较喜欢挂在mnt目录里，特征很明显



进入OpenWrt后最好安装一下这个，免得安装SSR PLUS报错

```
opkg install iptables iptables-mod-extra 
```
...
https://dl.openwrt.ai/packages-23.05/x86_64/kiddin9/
